...
The literature review began with learning the history of resume matching systems. Due to the commercial application, there have been systematic mathmatical approaches for informing hiring processes since at least the 1970s. However, many applicant variables such as demographic information aren't included in early models. Our tolerance for algorithmic mistakes should be lower than human error. - (DeLaCruz et all)​ Some of the earliest approaches for resume matching dependent on machine learning were relatively simple and relied on basic string matching, keyword extraction, and statistical methods but struggled with semantics much more than modern programs. - (Bocharova and Malakhov)​ 

To summarize the types of AI used in resume matching, TF-IDF and Jaccard Similarity are now seen as traditional methods for matching resumes with job postings. While not as sofisticated as newer approaches like BERT, they are easier to implement and faster. They are still an approach used by smaller enterprizes. Their key weaknesses are in their limited capacity for semantic understanding especially when it comes to similarities and synonyms. This is in part due to their dependence on matching keywords. Job seekers can boost their chance of resume selection in sytems using ​TF-IDF and Jaccard Similarity by tailoring their resumes to keywords in the job description. 

There are many proposals for new methods every year but perhaps the most widely used modern approach is BERT. WIth a focus on semantic understanding, it balances efficiency and accuracy​. However it's not above being fooled. It also requires more time and work to integrate and setup. ​

A middle ground between traditional and more advanced approaches for resume matching is a hybrid approach. The advantage of this is that it can capture semantic relationships and contextual understanding​ like BERT however it is the most computationally expensive with a risk of overfitting​.

In conslusion, BERT supported by FAISS proved to be the best-performing algorithm beating out Word2Vec, Jaccard Similarity and TF-IDF​ for its capacity to capture semantic and contextual relevance. We recommend using this approach for resume matching. 
